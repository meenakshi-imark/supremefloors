...
class Internal_create_account extends Controller
{
    public function index(){
        ...
        return view('backend.internal-create-account',compact('roles','staff_id'));
    }
    ...
    public function store(Request $request){
        ...
        $internal_cre_account->staff_id = $request->staff_id;
    ...
    public function show(Request $request){
       if(auth()->user()->role_id=="1"||auth()->user()->role_id=="2"){
    	...
    	$all_accounts = DB::table('users')->select('users.id','users.name','users.email','users.staff_id','users.username','users.gender','users.status','roles.name as rolename','roles.level','users.image','users.role_id','users.phone','users.dob','users.address','users.remarks')->leftJoin('roles', 'users.role_id', '=', 'roles.id')->where('users.name', 'LIKE',"%{$search}%")->orWhere('users.email', 'LIKE', "%{$search}%")->orWhere('users.username', 'LIKE', "%{$search}%")->orWhere('users.staff_id', 'LIKE', "%{$search}%")->orderBy('users.id','DESC')->paginate(10);
    	$all_accounts->appends(['search' => $search]);
        }
        elseif($request->input('filter')){
         $search = $request->input('filter');  
         $all_accounts = DB::table('users')->select('users.id','users.name','users.email','users.staff_id','users.username','users.gender','users.status','roles.name as rolename','roles.level','users.image','users.role_id','users.phone','users.dob','users.address','users.remarks')->leftJoin('roles', 'users.role_id', '=', 'roles.id')->where('users.role_id', $search)->orderBy('users.id','DESC')->paginate(10);
         $all_accounts->appends(['filter' => $search]);
        }
		else{
		$all_accounts = DB::table('users')->select('users.id','users.name','users.email','users.staff_id','users.username','users.gender','users.status','roles.name as rolename','roles.level','users.image','users.role_id','users.phone','users.dob','users.address','users.remarks')->leftJoin('roles', 'users.role_id', '=', 'roles.id')->orderBy('users.id','DESC')->paginate(10);
		}
		}
		else{
		...
		if($request->input('search')){
        $search = $request->input('search');
    	$all_accounts = DB::table('users')->select('users.id','users.name','users.creator_id','users.email','users.staff_id','users.username','users.gender','users.status','roles.name as rolename','roles.level','users.image','users.role_id','users.phone','users.dob','users.address','users.remarks')->leftJoin('roles', 'users.role_id', '=', 'roles.id')->where('users.creator_id',auth()->user()->id)->where('users.name', 'LIKE',"%{$search}%")->orderBy('users.id','DESC')->paginate(10);
    	 $all_accounts->appends(['search' => $search]);
        }	
		elseif($request->input('filter')){
         $search = $request->input('filter');  
         $all_accounts = DB::table('users')->select('users.id','users.name','users.email','users.staff_id','users.username','users.gender','users.status','roles.name as rolename','roles.level','users.image','users.role_id','users.phone','users.dob','users.address','users.remarks')->leftJoin('roles', 'users.role_id', '=', 'roles.id')->where('users.role_id', $search)->where('users.creator_id',auth()->user()->id)->orderBy('users.id','DESC')->paginate(10);
          $all_accounts->appends(['filter' => $search]);
        }	
		else{
		$all_accounts = DB::table('users')->select('users.id','users.name','users.creator_id','users.email','users.staff_id','users.username','users.gender','users.status','roles.name as rolename','roles.level','users.image','users.role_id','users.phone','users.dob','users.address','users.remarks')->leftJoin('roles', 'users.role_id', '=', 'roles.id')->where('users.creator_id',auth()->user()->id)->orderBy('users.id','DESC')->paginate(10);
		}
		}
