...
$(document).on('change', '.pro-select1', function() {
  ...
  var totalprice_val = $("#total_price").val();
  var totalprice_val1 = $(totalprice).val();
  var avoid_sign = "$";
  var new_totalprice = totalprice_val.replace(avoid_sign, '');
  ...
  var avoid_dollar =  $("#total").val().replace(avoid_sign, '');
  var finaltotal = parseInt(avoid_dollar,10)+parseInt(totalprice_new,10);
  ...
  $.ajax({
	     ...
	     success:function(data){
			...
			$("#append-products").append('<tr>'+
                                        ...
                                        '<td class="qty1-total'+count_td+'"></td>'+
                                    '</tr>');
		}
        });	
                        ...
                        <div class="col">
                            <div class="form-group form-select-group w-100">
                                ...
                                <div class="search-it">
                                    ...
                                    <div class="search-it">
                                    <i class="la la-search"></i>
									<input type="text" class="form-control attentionto" id="search" name="attentionto" placeholder="Search" required value="{{$attn}}" >
                            ...
                            <table>
								...
								@for($i=0;$i<count($arr['products']);$i++)
									...
									@endphp
                                       ...
                                       <td>${{$arr['price'][$i]}}</td>
                                       <td class="qty-t">${{$price_service}}</td>
									</tr>
								@endfor
                                </tbody>
                            </table>
...
$("#serviceprice").on("keyup", function(e) {
    ...
    $('td.qty-t').html(totalprice_new);
    $('td.qty-total').html(totalprice_new);
    var totalprice1 = $('#total_price').val();
    var newtotal = totalprice1.replace(avoid, '');
    var new_total = parseInt(serviceprice,10)+parseInt(newtotal,10);
    $('#total_price').val('$'+totalprice_new);
    $('#total').val('$'+totalprice_new);
        ...
        source: function( request, response ) {
          $.ajax({
            ...
            success: function( data ) {
                if(data==""){
                    $(".red-error").show();
                    $(".attentionto").css("border", "2px solid #ff0000");
                }
                else{
                    $(".red-error").hide();
                    $(".attentionto").css("border", "1px solid #ced4da");
                    response( data );
                }
            }
          });
